...
public class Shooter extends SubsystemBase {
  ...
  private final TalonFX flywheelMotorTopLeft = new TalonFX(Constants.FLYWHEEL_MOTOR_TOP_LEFT_CAN_ID,
      new CANBus(Constants.CANIVORE_NAME));
  // This motor is a Falcon 500
  private final TalonFX flywheelMotorTopRight = new TalonFX(Constants.FLYWHEEL_MOTOR_TOP_RIGHT_CAN_ID,
      new CANBus(Constants.CANIVORE_NAME));
  // This motor is a Falcon 500
  private final TalonFX flywheelMotorBottomLeft = new TalonFX(Constants.FLYWHEEL_MOTOR_BOTTOM_LEFT_CAN_ID,
      new CANBus(Constants.CANIVORE_NAME));
  // This motor is Falcon 500
  private final TalonFX flywheelMotorBottomRight = new TalonFX(Constants.FLYWHEEL_MOTOR_BOTTOM_RIGHT_CAN_ID,
      new CANBus(Constants.CANIVORE_NAME));
  private final LinkedMotors flywheelMotors = new LinkedMotors(flywheelMotorTopLeft, flywheelMotorTopRight,
      flywheelMotorBottomLeft, flywheelMotorBottomRight);
  // This motor is a Kraken x60
  private final TalonFX hoodRollerLeft = new TalonFX(Constants.HOOD_MOTOR_LEFT_CAN_ID,
      new CANBus(Constants.CANIVORE_NAME));
  // This motor is a Kraken x60
  private final TalonFX hoodRollerRight = new TalonFX(Constants.HOOD_MOTOR_RIGHT_CAN_ID,
      new CANBus(Constants.CANIVORE_NAME));
  // This motor is a Kraken x44
  private final TalonFX infeedMotorLeft = new TalonFX(Constants.INFEED_MOTOR_LEFT_CAN_ID,
      new CANBus(Constants.CANIVORE_NAME));
  // This motor is a Kraken x44
  private final TalonFX infeedMotorRight = new TalonFX(Constants.INFEED_MOTOR_RIGHT_CAN_ID,
      new CANBus(Constants.CANIVORE_NAME));
  ...
  public Shooter(Drivebase drivebase) {
    ...
    this.canFlywheel = flywheelMotorTopLeft.isConnected() && flywheelMotorTopRight.isConnected()
        && flywheelMotorBottomLeft.isConnected() && flywheelMotorBottomRight.isConnected();
  ...
  public void periodic() {
    // Flywheel is controlled by TestSubsystem via Constants; no dashboard reads
    // here.
    ...
    double measuredRps = flywheelMotorTopLeft.getRotorVelocity()
        .refresh()
        .getValueAsDouble();
    ...
    if (flywheelEnabled && canFlywheel) {
      ...
      this.flywheelMotorTopRight
          .setControl(new Follower(flywheelMotorTopLeft.getDeviceID(), MotorAlignmentValue.Opposed));
      this.flywheelMotorBottomLeft
          .setControl(new Follower(flywheelMotorTopLeft.getDeviceID(), MotorAlignmentValue.Aligned));
      this.flywheelMotorBottomRight
          .setControl(new Follower(flywheelMotorTopLeft.getDeviceID(), MotorAlignmentValue.Opposed));
    } else {
      targetRPS = 0;
      CoastOut req = new CoastOut();
  ...
  public void calculateLaunchValues(double distToGoal) {
    ...
    double estimatedTime = (verticalVel + Math.sqrt(verticalVel - 2 * (GRAVITY) * (GOAL_HEIGHT - FLYWHEEL_HEIGHT)))
        / GRAVITY;
    ...
    // double angleToFaceGoal = Math.atan2(yDiff, xDiff); I think this is done in a
    // different file
  ...
  public double getNeededFlywheelSpeed(double distToGoal) {
    return interpolateFlywheelSpeedByDistance(distToGoal) * FLYWHEEL_MAX_THEORETICAL_RPM; // Normalize the target
                                                                                          // velocity by the max
                                                                                          // theoretical
  }
  ...
  // Based on the hardcoded flywheel speed & distance tables and the robots
  // current distance away from the goal
  ...
  // Linear interpolate the hood angle between zero and one with the motor
  // rotations of up and down on the hood
  ...
  public double getFlywheelCurrent() {
    return flywheelMotorBottomLeft.getSupplyCurrent(true).getValueAsDouble()
        + flywheelMotorBottomRight.getSupplyCurrent(true).getValueAsDouble()
        + flywheelMotorTopLeft.getSupplyCurrent(true).getValueAsDouble()
        + flywheelMotorTopRight.getSupplyCurrent(true).getValueAsDouble();
  }
  ...
  public double getInfeedCurrent() {
    return infeedMotorLeft.getSupplyCurrent(true).getValueAsDouble()
        + infeedMotorRight.getSupplyCurrent(true).getValueAsDouble();
  }
  ...
  public double getHoodCurrent() {
    return hoodRollerLeft.getSupplyCurrent(true).getValueAsDouble()
        + hoodRollerRight.getSupplyCurrent(true).getValueAsDouble();
  }
  ...
  /**
   * Calculates the RPM for the flywheel based on the robot's pose and the hub's
   * pose.
   * 
  ...
  public double interpolateRPM(double distance) {
    ...
    if (distance < rpmTable[0][0])
      return rpmTable[0][1];
    if (distance > rpmTable[rpmTable.length - 1][0])
